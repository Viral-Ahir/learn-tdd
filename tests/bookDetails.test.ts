...
describe("showBookDtls", () => {
  ...
  it("should return null if id is not a string", async () => {
    await showBookDtls(res as Response, 123 as any);

    expect(res.status).toHaveBeenCalledWith(404);
    expect(res.send).toHaveBeenCalledWith("Book 123 not found");
  });

  it("should return 500 if book is not found", async () => {
    Book.findOne = jest.fn().mockReturnValue({
      populate: jest.fn().mockReturnThis(),
      exec: jest.fn().mockResolvedValue(null),
    });

    await showBookDtls(res as Response, "123");

    expect(res.status).toHaveBeenCalledWith(500);
    expect(res.send).toHaveBeenCalledWith("Error fetching book 123");
  });
});
